...
def encrypt(plainM, plainK):
    binM = format(int(''.join([bin(ord(plainM[x]))[2:].zfill(8) for x in range(len(plainM))]), 2), '064b')
    # binM = bin(int(plainM))[2:].zfill(64)
    binK = format(int(plainK, 2), '064b')
    ...
    # C = "".join([lastRL[x] for x in fp])

    # print(f"Part 2: {C}")
    return C
    # return format(int(C, 16), '064b')
